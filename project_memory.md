...
* **Manager Community Isolation & Strict Session RBAC Boundaries:** In `./app/admin/users/page.tsx` and `./app/api/admin/users/route.ts`, client requests pass explicit `Authorization: Bearer ${session.access_token}` headers. Server API GET, POST, PUT, and DELETE handlers extract user sessions via Supabase SSR and service role clients, guaranteeing that Managers (`role === 'manager'`) see and manage ONLY managers and editors belonging to their own assigned community (`community_id === currentUserCommunityId`).
