...
* **Mobile Navbar Enhancement:** In `./app/components/Navbar.tsx`, mobile menu includes **Home** (`/`) in `mobileNavLinks` list (01 Home, 02 Events, 03 Calendar, 04 Communities, 05 Support). The **Dashboard / Login** action CTA button is displayed on mobile top header bar and as a full-width CTA pill inside the mobile navigation menu overlay.
