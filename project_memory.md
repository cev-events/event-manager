...
* **Public Page Architecture & Nixtio Design System (Swiss Editorial Minimalism):**
  - **`app/page.tsx` (Landing Homepage):** Features an edge-to-edge dark hero section with smooth scroll shrink animation (`w-[99%]` to `w-[95%]`, `min-h-[99vh]` to `min-h-[75vh]`, `rounded-[1.3rem]` to `rounded-[1rem]`), oversized display title (`text-[15rem]` to `text-[10rem]`), 6-column Nixtio community logo card grid (`001 / Our Communities`), and 3-column schedule cards (`min-h-[680px]`) with poster image dark overlay, category badges, and hover arrow rotation (`group-hover:rotate-[-45deg]`).
  - **`app/components/Navbar.tsx` (Global Floating Navigation Bar):** Detects scroll bounds via `getBoundingClientRect()` relative to `#hero-section` (and default dark mode on `/calendar`). Auto-switches logo asset (`/cev_logo.svg` in white over dark backgrounds, `/cev_logo_b.svg` in black over light backgrounds). Features a 3-state transition: transparent spaced-out links at initial top, compact white glass floating pill (`bg-white/70 backdrop-blur-2xl border border-white/70 shadow-[0_8px_35px_rgba(0,0,0,0.10)]`) on scroll, and CTA button color switching (white on dark, black on light). Mobile menu includes numbered index links (`01 Events`, `02 Calendar`, etc.).
  - **`app/events/page.tsx` & `MasterCalendar.tsx` (Events Directory):** Configured container width `w-[96%] mx-auto` with clean centered loading spinner. Enforces strict public shielding: hides draft events (`status !== 'live'`) and suppresses `college schedule` & `exam` categories in public view. Renders `aspect-[3/4] rounded-[24px]` cards with poster hover zoom (`scale-105`), dark gradient overlay, category/status badges, YIQ high-contrast community labels, and rotating arrow buttons.
  - **`app/community/page.tsx` & `app/community/[id]/page.tsx` (Communities & Detail Pages):** List view features 3-column `min-h-[430px] rounded-[24px]` cards with `001` index badges, rounded logo container (`w-20 h-20`), title hover offset (`group-hover:translate-x-1`), and arrow button rotation. Community detail page features a `min-h-[560px] bg-white rounded-[28px]` hero with radial blur, `w-32 h-32` logo container, giant title (`text-8xl`), and an editorial events list (`01`, `02` index numbers, bold typography, hover slide arrow button).
  - **`app/support/page.tsx` (Support & Bug Report Page):** Cleaned up back link navigation. Form container styled with `bg-white rounded-[28px] p-8 sm:p-12 shadow-sm border border-neutral-200` and dark emerald/rose contrast badges. Calls `./app/api/support/route.ts` which inserts into Supabase `support_tickets` table and dispatches SMTP HTML emails to superadmins.
  - **`app/template.tsx` (Global Page Entrance Transitions):** Implemented global entrance animations with Framer Motion: `initial={{ opacity: 0, y: 24, filter: 'blur(5px)' }}`, `animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }}`, `transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}`.
