...
* **Mobile Scroll Optimization:** In `./app/components/Navbar.tsx` and `./app/page.tsx`, scroll listeners are optimized using `requestAnimationFrame` throttling and `{ passive: true }` event listeners. State updates are strictly diffed (`prev !== isNowScrolled`), preventing redundant re-renders on pixel-by-pixel scrolling. Mobile CSS transitions are hardware-accelerated and set to 300ms on touch devices (`duration-300 md:duration-[900ms]`), eliminating scroll jank on mobile phones while preserving full desktop smooth animations.
