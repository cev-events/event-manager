...
* **Mobile Public Event Slug Layout:** In `./app/events/[id]/page.tsx`, CSS grid orders poster container first (`order-1 lg:order-2 lg:col-span-5`) on mobile screens (`< lg`), placing the 3:4 aspect poster above event details, and content second (`order-2 lg:order-1 lg:col-span-7`). Retains 7-col content / 5-col poster on desktop.
* **Fast AI Engine & Modern Web Drawer Chat UI:** `./app/api/chat/route.ts` using `gemini-1.5-flash` with 4-second `AbortController` timeouts. `./app/components/EventAiDrawer.tsx` features `100dvh` dynamic mobile height, safe-area inset padding (`pb-safe`), live status pulse dot, conversation reset, and full Markdown parser (`renderFormattedMessage`) supporting headers, bold text, inline code blocks, bullet points, and numbered lists.
