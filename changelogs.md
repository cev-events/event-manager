...
- **Event Edit Modal Date Input Pre-population ([app/admin/events/page.tsx](./app/admin/events/page.tsx), [app/admin/my-community/events/page.tsx](./app/admin/my-community/events/page.tsx)):** Added `toIsoDateString` date parser helper to convert single-day and multi-day date strings into clean ISO 10-character `YYYY-MM-DD` strings. HTML `<input type="date">` inputs now correctly display and pre-populate start and end dates when editing events instead of showing `dd-mm-yyyy`.
